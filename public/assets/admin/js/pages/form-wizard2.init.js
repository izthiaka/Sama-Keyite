             ...
             transitionEffect: "slideLeft", onStepChanging: function (t, i, e) {
                 ...
                 onFinishing: function (t, i) {
                    ...
                    onFinished: function (t, i) {
                        ...
                        $.ajax({
                            type : 'POST',
                            url : "/sudo/reservation-aeroport-v",
                            data: $('#wizard-validation-form-2').serialize(),
                            success:function(data){
                                $('#wizard-validation-form-2').find('a[href="#finish"]').remove();
                                $('#wizard-validation-form-2').find('a[href="#previous"]').remove();
                                $('#wizard-validation-form-2 #steps-uid-1 .actions ul li:last-child').append('<div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div>');
